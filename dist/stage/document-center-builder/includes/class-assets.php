...
final class DCB_Assets {
    ...
    public static function enqueue_front_assets(): void {
        ...
        $has_digital_portal_shortcode = self::content_has_any_shortcode($content, array('dcb_digital_forms_portal', 'document_digital_forms_portal'));
        $has_upload_portal_shortcode = self::content_has_any_shortcode($content, array('dcb_upload_portal', 'document_upload_portal'));
        $requested_fillable_form = isset($_GET['dcb_form']) ? sanitize_key((string) $_GET['dcb_form']) : '';
        $should_enqueue_for_fillable_from_upload = $has_upload_portal_shortcode && $requested_fillable_form !== '';

        if (!$has_digital_portal_shortcode && !$should_enqueue_for_fillable_from_upload) {
            return;
        }
    ...
    public static function enqueue_upload_assets(): void {
        ...
        $max_batch_files = (int) get_option('dcb_upload_frontend_batch_size', 4);
        $max_batch_files = max(1, min(20, $max_batch_files));
