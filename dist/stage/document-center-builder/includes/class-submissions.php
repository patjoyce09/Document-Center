...
final class DCB_Submissions {
    public static function init(): void {
        ...
        add_action('admin_post_dcb_upload_artifact_file', array(__CLASS__, 'serve_upload_artifact_file'));
    ...
    public static function submission_row_actions(array $actions, WP_Post $post): array {
        ...
        if ($post->post_type === 'dcb_upload_log') {
            $upload_log_id = (int) $post->ID;
            $path = (string) get_post_meta($upload_log_id, '_dcb_upload_path', true);
            if ($path !== '') {
                $actions['dcb_view_artifact'] = '<a href="' . esc_url(self::upload_artifact_file_url($upload_log_id, 'inline')) . '">View File</a>';
                $actions['dcb_download_artifact'] = '<a href="' . esc_url(self::upload_artifact_file_url($upload_log_id, 'attachment')) . '">Download File</a>';
            }
            return $actions;
        }

        if ($post->post_type !== 'dcb_form_submission') {
            return $actions;
        }
    ...
    private static function upload_artifact_file_url(int $upload_log_id, string $disposition = 'inline'): string {
        $upload_log_id = max(0, $upload_log_id);
        $disposition = $disposition === 'attachment' ? 'attachment' : 'inline';
        $base = admin_url('admin-post.php?action=dcb_upload_artifact_file&upload_log_id=' . $upload_log_id . '&disposition=' . $disposition);
        return wp_nonce_url($base, 'dcb_upload_artifact_file_' . $upload_log_id, 'dcb_upload_artifact_nonce');
    }

    public static function serve_upload_artifact_file(): void {
        if (!DCB_Permissions::can(DCB_Permissions::CAP_REVIEW_SUBMISSIONS)) {
            wp_die('Unauthorized');
        }

        $upload_log_id = isset($_GET['upload_log_id']) ? max(0, (int) $_GET['upload_log_id']) : 0;
        if ($upload_log_id < 1) {
            wp_die('Missing upload artifact ID.');
        }

        if (!isset($_GET['dcb_upload_artifact_nonce']) || !wp_verify_nonce((string) $_GET['dcb_upload_artifact_nonce'], 'dcb_upload_artifact_file_' . $upload_log_id)) {
            wp_die('Security check failed.');
        }

        $path = (string) get_post_meta($upload_log_id, '_dcb_upload_path', true);
        $mime = sanitize_text_field((string) get_post_meta($upload_log_id, '_dcb_upload_mime', true));
        $name = sanitize_file_name((string) get_post_meta($upload_log_id, '_dcb_upload_title', true));
        ...
        if (!is_string($real) || $real === '' || !is_string($base_real) || $base_real === '' || strpos($real, trailingslashit($base_real)) !== 0) {
            wp_die('Invalid file location.');
        }

        if ($mime === '') {
            $detected = wp_check_filetype($real);
            $mime = sanitize_text_field((string) ($detected['type'] ?? 'application/octet-stream'));
            if ($mime === '') {
        ...
        $disposition = isset($_GET['disposition']) && (string) $_GET['disposition'] === 'attachment' ? 'attachment' : 'inline';
