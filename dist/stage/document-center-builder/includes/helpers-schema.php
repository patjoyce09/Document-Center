...
function dcb_parse_emails(string $emails): array {
    $normalized = strtolower((string) $emails);
    $parts = preg_split('/[\s,;]+/', $normalized) ?: array();
...
function dcb_normalize_ocr_meta($meta): array {
    ...
    if (isset($meta['geometry']) && is_array($meta['geometry'])) {
        $out['geometry'] = array(
            'x' => round(max(0.0, min(1.0, (float) ($meta['geometry']['x'] ?? 0.0))), 4),
            'y' => round(max(0.0, min(1.0, (float) ($meta['geometry']['y'] ?? 0.0))), 4),
            'w' => round(max(0.0, min(1.0, (float) ($meta['geometry']['w'] ?? 0.0))), 4),
            'h' => round(max(0.0, min(1.0, (float) ($meta['geometry']['h'] ?? 0.0))), 4),
            'unit' => sanitize_key((string) ($meta['geometry']['unit'] ?? 'page_ratio')),
        );
    }
    return $out;
}
...
function dcb_normalize_single_form(array $form): ?array {
    ...
    if (isset($form['digital_twin_background']) && is_array($form['digital_twin_background'])) {
        $bg = (array) $form['digital_twin_background'];
        $url = esc_url_raw((string) ($bg['image_url'] ?? ''));
        if ($url !== '') {
            $normalized_form['digital_twin_background'] = array(
                'image_url' => $url,
                'mode' => sanitize_key((string) ($bg['mode'] ?? 'overlay')),
                'opacity' => round(max(0.0, min(1.0, (float) ($bg['opacity'] ?? 1.0))), 4),
            );
            if (isset($bg['pages']) && is_array($bg['pages'])) {
                $pages = array();
                foreach ($bg['pages'] as $page_row) {
                    if (!is_array($page_row)) {
                        continue;
                    }
                    $p_url = esc_url_raw((string) ($page_row['image_url'] ?? ''));
                    if ($p_url === '') {
                        continue;
                    }
                    $pages[] = array(
                        'page_number' => max(1, (int) ($page_row['page_number'] ?? (count($pages) + 1))),
                        'image_url' => $p_url,
                    );
                }
                if (!empty($pages)) {
                    $normalized_form['digital_twin_background']['pages'] = $pages;
                }
            }
        }
    }
...
function dcb_form_definitions(bool $for_js = false): array {
    ...
    foreach ($forms as $key => $form) {
        $out[$key] = array(
            ...
            'template_blocks' => isset($form['template_blocks']) && is_array($form['template_blocks']) ? array_values($form['template_blocks']) : array(),
            'documentNodes' => isset($form['document_nodes']) && is_array($form['document_nodes']) ? array_values($form['document_nodes']) : array(),
            'document_nodes' => isset($form['document_nodes']) && is_array($form['document_nodes']) ? array_values($form['document_nodes']) : array(),
            ...
            'ocr_review' => isset($form['ocr_review']) && is_array($form['ocr_review']) ? $form['ocr_review'] : array(),
            'digital_twin_hints' => isset($form['digital_twin_hints']) && is_array($form['digital_twin_hints']) ? $form['digital_twin_hints'] : array(),
            'digitalTwinHints' => isset($form['digital_twin_hints']) && is_array($form['digital_twin_hints']) ? $form['digital_twin_hints'] : array(),
            'digital_twin_background' => isset($form['digital_twin_background']) && is_array($form['digital_twin_background']) ? $form['digital_twin_background'] : array(),
            'digitalTwinBackground' => isset($form['digital_twin_background']) && is_array($form['digital_twin_background']) ? $form['digital_twin_background'] : array(),
        );
    }
