...
== Changelog ==
= 0.3.12 =
* Improved OCR-to-digital-twin rendering so fillable forms better preserve uploaded document structure using template blocks, document nodes, and layout hints.
* Added OCR review workbench layout fidelity preview scoring (confidence + hint/node/section coverage) before applying draft fields.
* Persisted fidelity preview metadata into OCR review payload on draft apply for safer publish decisions.
* Fixed OCR draft label regression where draft form title could be overwritten by field-label loop state.
* Hardened OCR draft apply key/structure normalization to avoid duplicate keys and stale section/document-node references.
